...
export default function SitemapExplorer({ sitemaps }) {
  ...
  return (
    <section className="min-w-0 overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--panel)]" aria-labelledby="sitemap-explorer-title">
      <header className="relative border-b border-[var(--border)] px-5 py-4 2xl:pr-36"><div><p className="m-0 text-[11px] font-bold uppercase tracking-[.14em] text-[var(--accent-blue)]">Discovery inventory</p><h2 id="sitemap-explorer-title" className="mt-2 text-base font-bold text-[var(--text)]">3. Sitemap Explorer</h2><p className="mt-1 text-[13px] leading-5 text-[var(--text-secondary)]">Browse returned URLs in a bounded, scrollable inventory; each page contains up to {PAGE_SIZE} records.</p></div>{validSitemaps.length > 0 && <span className="mt-3 inline-flex rounded-full border border-emerald-300/20 bg-emerald-300/[.07] px-3 py-2 text-[10px] font-bold uppercase tracking-[.06em] text-emerald-100 2xl:absolute 2xl:right-5 2xl:top-4 2xl:mt-0">{validSitemaps.length} valid sitemap{validSitemaps.length === 1 ? '' : 's'}</span>}</header>
