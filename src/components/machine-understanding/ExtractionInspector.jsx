    ...
    <SectionShell id="extraction-inspector" number="3" title="Extraction Inspector" description="The actual content and machine-readable data produced by this scan.">
      <div className="p-5">
        <div className="flex gap-1 overflow-x-auto rounded-xl border border-[var(--border)] bg-[var(--bg-darker)] p-1" role="tablist" aria-label="Extraction formats">
          {tabs.map(tab => <button key={tab.id} id={`tab-${tab.id}`} type="button" role="tab" aria-selected={activeId === tab.id} aria-controls={`panel-${tab.id}`} onClick={() => setActiveId(tab.id)} className={`relative min-h-10 whitespace-nowrap rounded-lg px-3 text-[12px] font-medium transition-all focus-visible:outline focus-visible:outline-2 focus-visible:outline-[var(--accent-purple)] ${activeId === tab.id ? 'bg-[var(--accent-purple)]/10 text-purple-100 shadow-[inset_0_0_0_1px_rgba(155,109,255,.2)]' : 'text-[var(--text-secondary)] hover:bg-white/[.025] hover:text-[var(--text)]'}`}>{tab.label}{activeId === tab.id && <span className="mu-tab-indicator absolute inset-x-3 -bottom-1 h-px bg-[var(--accent-purple)]" />}</button>)}
        </div>
        <div className="mu-stagger-grid mu-no-sheen mt-4 grid gap-4 xl:grid-cols-[minmax(0,1fr)_300px]">
          <div className="relative overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--bg-darker)]">
            <div className="flex items-center justify-between border-b border-[var(--border)] px-4 py-3"><span className="flex items-center gap-2 text-[11px] font-medium uppercase tracking-[0.1em] text-purple-100"><ScanLine size={16} />{active.label} extraction</span><button type="button" onClick={copyContent} className={`inline-flex min-h-10 items-center gap-2 rounded-lg border px-3 text-[12px] transition ${copied ? 'border-[var(--accent-teal)]/20 bg-[var(--accent-teal)]/[.06] text-emerald-100' : 'border-[var(--border)] text-[var(--text-secondary)] hover:border-[var(--accent-purple)]/25 hover:text-purple-100'}`}>{copied ? <Check size={16} /> : <Clipboard size={16} />}{copied ? 'Copied' : 'Copy'}</button></div>
          ...
          <aside className="rounded-xl border border-[var(--accent-purple)]/15 bg-[var(--bg-darker)]/55 p-5">
            ...
            <strong className="mt-2 block text-xl font-semibold tracking-[-0.02em] text-purple-100">{quality.label}</strong>
            ...
            <div className="mt-5 grid gap-3">{quality.checks.map(([label, passed]) => <div key={label} className="flex items-start gap-3"><span className={`mt-0.5 grid size-6 shrink-0 place-items-center rounded-full ${passed ? 'bg-[var(--accent-teal)]/10 text-emerald-100' : 'bg-white/[.035] text-[var(--text-muted)]'}`}>{passed ? <Check size={14} /> : '—'}</span><span className={`text-[12px] leading-5 ${passed ? 'text-[var(--text)]' : 'text-[var(--text-secondary)]'}`}>{label}</span></div>)}</div>
          </aside>
        </div>
      </div>
    </SectionShell>
