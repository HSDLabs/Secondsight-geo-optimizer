...
const cardClass = 'mu-hover-card flex min-w-0 flex-col overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--bg-darker)]/45 p-5 transition duration-200 hover:-translate-y-0.5 hover:border-white/15'
    ...
    <>
      <SectionShell id="raw-evidence" icon={Image} title="Raw Evidence" description="The machine-readable signals found on this page.">
        <div className="mu-stagger-grid grid items-stretch gap-4 p-5 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <article className={`${cardClass} sm:col-span-2 xl:col-span-4`}>
...
function TreeDialog({ open, data, treeMode, setTreeMode, selectedNodeId, selectedPath, onSelectNode, onClose }) {
  return (
    <AnalysisModal open={open} onClose={onClose} title="Accessibility tree" description="Inspect the linked DOM outline or Playwright's native ARIA snapshot.">
      <div className="mb-4 flex w-fit rounded-lg border border-[var(--border)] bg-[var(--bg-darker)] p-1"><button type="button" onClick={() => setTreeMode('outline')} className={`min-h-10 rounded-md px-3 text-[12px] ${treeMode === 'outline' ? 'bg-[var(--accent-blue)]/10 text-[var(--status-info)]' : 'text-[var(--text-secondary)]'}`}>DOM outline</button><button type="button" onClick={() => setTreeMode('aria')} disabled={!data?.a11y?.ariaSnapshot} className={`min-h-10 rounded-md px-3 text-[12px] disabled:text-[var(--text-muted)] ${treeMode === 'aria' ? 'bg-[var(--accent-purple)]/10 text-[var(--status-purple)]' : 'text-[var(--text-secondary)]'}`}>ARIA snapshot</button></div>
...
function CardTitle({ icon: Icon, children }) {
  return <h3 className="flex items-center gap-3 text-[14px] font-semibold text-[var(--text)]"><span className="grid size-10 shrink-0 place-items-center rounded-xl border border-[var(--accent-blue)]/20 bg-[var(--accent-blue)]/[.07] text-[var(--status-info)]"><Icon size={20} strokeWidth={1.7} /></span><span className="min-w-0 break-words">{children}</span><Info size={15} className="shrink-0 text-[var(--text-secondary)]" /></h3>
}
...
function StatusRows({ rows }) {
  return <dl className="mt-4 mb-4 min-w-0">{rows.map(([label, present, detail]) => <div key={label} className={rowClass}><dt className="min-w-0 text-[var(--text-secondary)]">{label}</dt><dd className="flex min-w-0 items-center justify-end">{present && detail ? <span className="max-w-36 truncate text-right text-[11px] font-medium text-[var(--status-good)]" title={detail}>{detail}</span> : present ? <Check size={17} strokeWidth={2.2} className="text-[var(--accent-teal)]" /> : <span className="inline-flex items-center gap-1.5 rounded-full border border-[var(--accent-amber)]/25 bg-[var(--accent-amber)]/10 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.06em] text-[var(--status-warning)]"><AlertTriangle size={13} />Issue</span>}</dd></div>)}</dl>
}
...
function MetadataFooter({ metadata, schemaTypes }) {
  ...
  return <div className="mt-auto pt-4"><p className={`text-[12px] ${issueCount ? 'text-[var(--status-warning)]' : 'text-[var(--text-secondary)]'}`}>{issueCount ? `${issueCount} metadata issue${issueCount > 1 ? 's' : ''} found` : 'All critical metadata found'}</p><button type="button" onClick={() => document.getElementById('extraction-inspector')?.scrollIntoView({ behavior: 'smooth', block: 'start' })} className={`${actionClass} mt-3`}>View details <ExternalLink size={15} /></button></div>
}

function Signal({ label, value, good }) {
  return <div className="inline-flex min-w-0 items-center gap-2 rounded-full border border-[var(--border)] bg-[var(--panel-raised)]/50 px-3 py-2.5"><span className={`size-2 shrink-0 rounded-full ${good ? 'bg-[var(--accent-teal)]' : 'bg-[var(--text-muted)]'}`} /><span className="text-[11px] text-[var(--text-secondary)]">{label}</span><strong className={`text-[11px] font-medium ${good ? 'text-[var(--status-good)]' : 'text-[var(--text)]'}`}>{value}</strong></div>
}
